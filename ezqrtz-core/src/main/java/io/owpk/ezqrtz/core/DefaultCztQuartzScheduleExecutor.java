...
import io.owpk.ezqrtz.core.model.RepeatTriggerDefinition;
...
public class DefaultCztQuartzScheduleExecutor implements CztQuartzScheduleExecutor {
    ...
    public boolean deleteJob(String identity) {
        return trySafe(() -> scheduler.deleteJob(namespace.jobKey(identity)), _ -> false);
    }

    @Override
    public boolean jobExists(String identity) {
        return trySafe(() -> scheduler.checkExists(namespace.jobKey(identity)), _ -> false);
    }

    @Override
    public boolean triggerExists(String identity) {
        return trySafe(() -> scheduler.checkExists(namespace.triggerKey(identity)), _ -> false);
    }
